class DocumentFormatter {
  static formatMarkdown(content) {
    let result = content;
    result = result.replace(/^# (.*)$/gm, "\x1b[41m$1\x1b[0m"); // # — красный фон
    result = result.replace(/^## (.*)$/gm, "\x1b[42m$1\x1b[0m"); // ## — зелёный фон
    result = result.replace(/^### (.*)$/gm, "\x1b[43m$1\x1b[0m"); // ### — жёлтый фон
    result = result.replace(/^#### (.*)$/gm, "\x1b[44m$1\x1b[0m"); // #### — синий фон
    result = result.replace(/^##### (.*)$/gm, "\x1b[45m$1\x1b[0m"); // ##### — пурпурный фон
    result = result.replace(/^###### (.*)$/gm, "\x1b[46m$1\x1b[0m");
    result = result.replace(/\*\*(.*?)\*\*/g, "\x1b[1m$1\x1b[0m"); // Жирный
    result = result.replace(/\*(.*?)\*/g, "\x1b[3m$1\x1b[0m"); // Курсив
    result = result.replace(/__(.*?)__/g, "\x1b[4m$1\x1b[0m"); // Подчёркнутый
